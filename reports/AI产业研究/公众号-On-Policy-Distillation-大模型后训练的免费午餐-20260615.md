...
[图1：论文 Figure 1——GKD 在三个任务上全面超越传统方法]

把两种方法的特点列成表，空白区域一目了然：

| 方法 | 采样方式 | 奖励信号 |
|------|--------|--------|
| 监督微调（SFT） | 离策略（学教师的文本） | 密集（每个词都有监督） |
| 强化学习（RL） | 在策略（学自己的文本） | 稀疏（只有最终分数） |
| **OPD（本文主角）** | **在策略（学自己的文本）** | **密集（每个词都有教师反馈）** |
...
> D_KL(P_teacher || P_student) = Σ P_teacher(x) · log[ P_teacher(x) / P_student(x) ]
...
> D_KL(P_student || P_teacher) = Σ P_student(x) · log[ P_student(x) / P_teacher(x) ]

大白话：你复习的每一个考点必须是对的，但可以不覆盖所有考点。学生选中的每个方向，必须确保教师也认可。

效果：学生会集中力量学好一种最优策略，而不是分散地覆盖所有可能。术语叫"模式寻找"。

[图2：论文 Figure 4——正向 KL vs 逆向 KL 的质量-多样性权衡]
...
[图4：论文 Figure 9——GSM8K 数学推理对比，OPD 全面超越所有方法]

但最震撼的发现是关于数据效率的：

[图5：论文 Figure 3——数据效率对比]
...
[图6：论文 Figure 10——MMLU/BBH 通用能力对比]
...
**配图说明：**
- [图1]：论文 Figure 1（第1页）——GKD 与传统方法在摘要/翻译/推理三个任务上的性能对比
- [图2]：论文 Figure 4（第6页）——正向 KL vs 逆向 KL 的质量-多样性权衡
- [图3]：论文 Algorithm 1（第4页）——GKD 算法伪代码
- [图4]：论文 Figure 9（第8页）——GSM8K 数学推理对比
- [图5]：论文 Figure 3（第6页）——数据效率对比（5%数据超过100%）
- [图6]：论文 Figure 10（第9页）——MMLU/BBH 通用能力对比

**论文原文：**
- PDF 已下载至 `~/OPD论文/GKD-OPD-ICLR2024.pdf`
- arXiv: https://arxiv.org/abs/2306.13649
- 同目录下还有 MiniLLM 和 OPD Survey 两篇参考论文
